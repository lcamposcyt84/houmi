...
export function phpUrl(path: string): string {
  // If path contains a query string (e.g. "reviews/get?productId=xxx"),
  // we need to insert .php before the ?, not at the end.
  const qIdx = path.indexOf("?");
  if (qIdx !== -1) {
    const base = path.slice(0, qIdx);
    const query = path.slice(qIdx);
    return `${PHP_API_URL}/${base}.php${query}`;
  }
  return `${PHP_API_URL}/${path}.php`;
}
