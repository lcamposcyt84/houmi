"use client";

import { useState } from "react";
import Link from "next/link";
import { Card, Button } from "@/components/ui";
import { Search, Plus, Package, Eye } from "lucide-react";

interface PurchaseItem {
  id: string;
  productName: string;
  quantity: number;
  costUsd: number;
}

interface Purchase {
  id: string;
  supplier: string;
  description: string | null;
  totalUsd: number;
  totalVes: number | null;
  status: string;
  createdAt: Date;
  items: PurchaseItem[];
}

interface PurchasesTableProps {
  purchases: Purchase[];
  exchangeRate: number;
}

export function PurchasesTable({ purchases, exchangeRate }: PurchasesTableProps) {
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");

  const filteredPurchases = purchases.filter((purchase) => {
    const matchesSearch = purchase.supplier
      .toLowerCase()
      .includes(search.toLowerCase());
    const matchesStatus =
      statusFilter === "all" || purchase.status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const formatDate = (date: Date) => {
    return new Date(date).toLocaleDateString("es-VE", {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });
  };

  const getStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      received: "bg-green-100 text-green-700",
      pending: "bg-yellow-100 text-yellow-700",
      cancelled: "bg-red-100 text-red-700",
    };
    const labels: Record<string, string> = {
      received: "Recibida",
      pending: "Pendiente",
      cancelled: "Cancelada",
    };
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium ${
          styles[status] || "bg-gray-100 text-gray-700"
        }`}
      >
        {labels[status] || status}
      </span>
    );
  };

  const totalUsd = filteredPurchases.reduce((sum, p) => sum + p.totalUsd, 0);

  return (
    <div className="space-y-4">
      {/* Filters */}
      <Card className="p-4">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1 relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
            <input
              type="text"
              placeholder="Buscar por proveedor..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20"
            />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="px-4 py-2 border border-gray-200 rounded-lg focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20"
          >
            <option value="all">Todos los estados</option>
            <option value="pending">Pendientes</option>
            <option value="received">Recibidas</option>
            <option value="cancelled">Canceladas</option>
          </select>
          <Link href="/admin/purchases/new">
            <Button variant="primary" leftIcon={<Plus className="w-4 h-4" />}>
              Nueva Compra
            </Button>
          </Link>
        </div>
      </Card>

      {/* Summary */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-4">
          <p className="text-sm text-brand-text-muted">Total Compras</p>
          <p className="text-2xl font-bold text-brand-text">
            {filteredPurchases.length}
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-sm text-brand-text-muted">Total USD</p>
          <p className="text-2xl font-bold text-blue-600">
            ${totalUsd.toFixed(2)}
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-sm text-brand-text-muted">Total VES</p>
          <p className="text-2xl font-bold text-brand-primary">
            Bs {(totalUsd * exchangeRate).toFixed(2)}
          </p>
        </Card>
      </div>

      {/* Table */}
      <Card className="overflow-hidden">
        {filteredPurchases.length === 0 ? (
          <div className="text-center py-12">
            <Package className="w-12 h-12 text-gray-300 mx-auto mb-3" />
            <p className="text-brand-text-muted">No hay compras registradas</p>
            <Link href="/admin/purchases/new" className="mt-4 inline-block">
              <Button variant="outline" size="sm">
                Registrar primera compra
              </Button>
            </Link>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left px-4 py-3 text-sm font-medium text-brand-text-muted">
                    Fecha
                  </th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-brand-text-muted">
                    Proveedor
                  </th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-brand-text-muted">
                    Productos
                  </th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-brand-text-muted">
                    Total
                  </th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-brand-text-muted">
                    Estado
                  </th>
                  <th className="text-right px-4 py-3 text-sm font-medium text-brand-text-muted">
                    Acciones
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {filteredPurchases.map((purchase) => (
                  <tr key={purchase.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-brand-text-muted">
                      {formatDate(purchase.createdAt)}
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-medium text-brand-text">
                        {purchase.supplier}
                      </p>
                      {purchase.description && (
                        <p className="text-xs text-brand-text-muted">
                          {purchase.description}
                        </p>
                      )}
                    </td>
                    <td className="px-4 py-3 text-brand-text-muted">
                      {purchase.items.length} producto(s)
                    </td>
                    <td className="px-4 py-3">
                      <div>
                        <p className="font-medium text-brand-text">
                          ${purchase.totalUsd.toFixed(2)}
                        </p>
                        <p className="text-xs text-brand-text-muted">
                          Bs{" "}
                          {(purchase.totalVes ||
                            purchase.totalUsd * exchangeRate).toFixed(2)}
                        </p>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {getStatusBadge(purchase.status)}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <Link href={`/admin/purchases/${purchase.id}`}>
                        <Button variant="ghost" size="sm">
                          <Eye className="w-4 h-4" />
                        </Button>
                      </Link>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </Card>
    </div>
  );
}
